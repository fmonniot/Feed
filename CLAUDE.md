...
2 tests are `@Ignore`'d (PullToRefresh gesture requires a real device). Confirm 0 failures. The gradle build automatically runs `cargo build` on the server first via the `:app:buildServerBinary` task. Pass `-PskipServerBuild` to skip rebuilding if you know the binary at `server/target/debug/server` is current.
...
Covers `SessionManager`, `ServerUrlStore.normalizeServerUrl`, `RelativeTime`, and `FeedViewModel` behavior. Confirm 0 failures.
...
- [TICKETS.md](TICKETS.md) — full feature/infra backlog with numeric ticket IDs (`#1`–`#135`). Reference tickets by their number.
- [BUGS.md](BUGS.md) — full bug backlog with IDs (`BUG-1`–`BUG-62`). Reference bugs by their ID.
...
- **AGP 9.2 KMP library plugin.** The `shared/` module uses `com.android.kotlin.multiplatform.library`, not `com.android.library`. The Gradle DSL is different — use `android { }` *nested inside* `kotlin { }` instead of the classic top-level `android { }` block (same nesting position, different block name than the classic plugin). Note: AGP 9.0 named this nested block `androidLibrary { }`; AGP 9.2 renamed it to `android { }` (same `KotlinMultiplatformAndroidLibraryExtension` type, `androidLibrary` now deprecated) — see ticket #111.
