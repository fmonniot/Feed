...
- **Root cause:** `FeedBottomSheet` is a hand-rolled full-screen `Dialog` (chosen over Material3's `ModalBottomSheet` for exact VISUAL_SPEC chrome control, per the doc comment at line 55-62). It applies a fixed `bottom = 30.dp` content padding (line 114) and never consumes `WindowInsets.navigationBars`, so the sheet's background `Column` isn't drawn edge-to-edge behind the system nav bar the way Material3's own bottom sheet would handle for free.
- **Fix:** Split the sheet's single `Column` into two: an outer, unpadded `Column` that carries the `background`/`border`/rounded-corner shape and is sized by its children (no longer clipped short by a fixed bottom padding), and an inner `Column` that keeps the exact original `10dp`/`30dp` top/bottom padding around the grab handle, title, content, and button row — so none of those elements move from their previously-correct on-screen position. A new trailing `Spacer(Modifier.windowInsetsBottomHeight(WindowInsets.navigationBars))` is appended as a sibling *after* the inner padded `Column`, inside the outer one. Because the outer `Column`'s background/border paint using its own final measured size (inner padded column + trailing spacer), the sheet's background now extends exactly as far as the live system nav-bar inset, filling the area that used to show the scrim through a gap. The app already calls `enableEdgeToEdge()` in `MainActivity` and the sibling `MainTabShell`'s tab bar already reads `WindowInsets.navigationBars` the same way, so this Dialog window receives real (non-zero) nav-bar insets to size the spacer from.
- **Validation:** Manual verification for the visual/inset behavior (layout against the real system nav bar isn't practical to unit-test with Robolectric) — see the reasoning above: the trailing spacer is sized from the live `WindowInsets.navigationBars` inset, so it self-adjusts for both 3-button nav (small/zero inset) and gesture nav (taller inset) without a hardcoded guess, and it's appended *after* the untouched, already-padded content, so button tap targets keep their exact prior position. Regression coverage: `./gradlew :app:testDebugUnitTest` run in full after the change (see PR for pass count), including the existing `SubscriptionsScreenTest.kt` tests that render `FeedBottomSheet` transitively through every concrete sheet (`addFeedDialog_*`, `fetchIntervalDialog_*`, `renameFeedSheet_*`, `newCategorySheet_*`/`renameCategorySheet_*`/`deleteCategorySheet_*`, `moveToCategorySheet_*`), plus `fetchIntervalDialog_showsPresetsWithCurrentSelected` and `moveToCategorySheet_manyCategories_allOptionsExistAndPrimaryButtonWorks`, which specifically assert on the `sheet_content$testTagSuffix` node preserved by this change.
