...
- **Root cause:** The app surfaced parse errors (and other feed status errors) via snackbars or inline notifications, rather than deferring to the persistent sidebar/feed-list status indicator. Each sync cycle that encountered a parse-error feed triggered a new notification, even if the error was pre-existing and hadn't changed.
...
### BUG-59: Bottom sheets don't extend behind the system gesture navigation bar

- **Status:** NEEDS DEVICE VERIFICATION
- **Module:** `app/`
- **Files:** `app/src/main/java/eu/monniot/feed/ui/subs/FeedBottomSheet.kt` (shared shell for every #124 bottom-sheet flow — `FeedBottomSheet` composable, sheet `Column` at line ~102-115)
- **Symptom:** Every bottom sheet (Add feed, Move to category…, Rename feed, Fetch interval, New category, Rename category, Delete category) visually stops short of the bottom of the screen, leaving a gap above the system gesture navigation bar instead of the sheet's background extending behind it. Purely a rendering issue — the sheet's tap targets (Cancel/primary buttons) are already positioned correctly and remain reachable.
- **Root cause:** `FeedBottomSheet` is a hand-rolled full-screen `Dialog` (chosen over Material3's `ModalBottomSheet` for exact VISUAL_SPEC chrome control, per the doc comment at line 55-62). It applies a fixed `bottom = 30.dp` content padding (line 114) and never consumes `WindowInsets.navigationBars`, so the sheet's background `Column` isn't drawn edge-to-edge behind the system nav bar the way Material3's own bottom sheet would handle for free. Separately, a `Dialog` creates its own window with its own decor; with `DialogProperties.decorFitsSystemWindows` left at its default (`true`), that window's decor fits system windows and `WindowInsets.navigationBars` reads 0 *inside the dialog*, regardless of what the host Activity does — `enableEdgeToEdge()` in `MainActivity` and `MainTabShell` reading real insets both concern the Activity's window, not this Dialog's.
- **Fix:** Split the sheet's single `Column` into two: an outer, unpadded `Column` that carries the `background`/`border`/rounded-corner shape and is sized by its children (no longer clipped short by a fixed bottom padding), and an inner `Column` that keeps the exact original `10dp`/`30dp` top/bottom padding around the grab handle, title, content, and button row — so none of those elements move from their previously-correct on-screen position. A new trailing `Spacer(Modifier.windowInsetsBottomHeight(WindowInsets.navigationBars))` is appended as a sibling *after* the inner padded `Column`, inside the outer one. Because the outer `Column`'s background/border paint using its own final measured size (inner padded column + trailing spacer), the sheet's background now extends exactly as far as the live system nav-bar inset, filling the area that used to show the scrim through a gap. Also set `DialogProperties(decorFitsSystemWindows = false)` on the sheet's `Dialog` — without it, `WindowInsets.navigationBars` reads 0 inside the dialog's window and the new spacer is a no-op regardless of the Activity's edge-to-edge setup.
- **Validation:** Manual verification for the visual/inset behavior (layout against the real system nav bar isn't practical to unit-test with Robolectric, and Robolectric reports `WindowInsets.navigationBars` as 0 regardless of `decorFitsSystemWindows`, so the JVM suite can't distinguish a working fix from a no-op one here). The trailing spacer is sized from the live `WindowInsets.navigationBars` inset, so it should self-adjust for both 3-button nav (which still reports its full ~48dp inset under SDK 35+ edge-to-edge enforcement, not a "small/zero" one) and gesture nav, and it's appended *after* the untouched, already-padded content, so button tap targets keep their exact prior position — but this has **not yet been confirmed on a gesture-nav device or emulator**; no `adb`/emulator was available in this environment to capture a before/after screenshot. Regression coverage: `./gradlew :app:testDebugUnitTest` run in full after the change (see PR for pass count), including the existing `SubscriptionsScreenTest.kt` tests that render `FeedBottomSheet` transitively through every concrete sheet (`addFeedDialog_*`, `fetchIntervalDialog_*`, `renameFeedSheet_*`, `newCategorySheet_*`/`renameCategorySheet_*`/`deleteCategorySheet_*`, `moveToCategorySheet_*`), plus `fetchIntervalDialog_showsPresetsWithCurrentSelected` and `moveToCategorySheet_manyCategories_allOptionsExistAndPrimaryButtonWorks`, which specifically assert on the `sheet_content$testTagSuffix` node preserved by this change. **Before marking FIXED:** verify on a gesture-nav device/emulator that the sheet background now reaches the bottom of the screen.

---

### BUG-60: "Change Feed URL" popup uses a centered AlertDialog instead of the bottom-sheet pattern

- **Status:** FIXED
- **Module:** `app/` + spec files
- **Files:**
  - `app/src/main/java/eu/monniot/feed/ui/subs/FeedDialogs.kt` — `ChangeUrlDialog` rebuilt on the shared `FeedBottomSheet` shell (same pattern as `RenameFeedSheet`), keeping the `change_url_input`/`change_url_error`/`change_url_save` test tags and the existing pre-fill/validation/inline-error behavior. `DeleteConfirmDialog` (Unsubscribe) is now the only inline `AlertDialog` left in this file.
  - `app/src/test/java/eu/monniot/feed/ui/subs/SubscriptionsScreenTest.kt` — extended the BUG-56 Change-URL coverage to assert the bottom-sheet shell's own tags (`sheet_change_url`, `sheet_cancel_change_url`) instead of `AlertDialog` semantics, and added a new cancel-flow test.
  - `spec/VISUAL_SPEC.md` §Bottom sheets — added Change URL to the sheet-flow list and the per-feed overflow-menu item list, plus a bullet describing its pre-fill/inline-error behavior.
  - `spec/story-board/prototypes/subscriptions.jsx` — added a `Change URL…` `mitem` to `feedMenuEl` (next to `Rename…`), a `changeUrl` action in `useSubActions`, and a `sheet.type === 'changeUrl'` case rendering an `SubMSheet` with a pre-filled URL input, matching the `renameFeed` sheet's shape.
- **Symptom:** Every other per-feed/category edit surface (Move to category…, Rename feed, Fetch interval, New/Rename category, Delete category) opens as a bottom sheet via the shared `FeedBottomSheet` shell (#124), but "Change URL" — reachable from the same per-feed overflow (⋯) menu — pops up as a centered Material3 `AlertDialog`, which is visually and interactionally inconsistent with the rest of the overflow menu.
- **Root cause:** `ChangeUrlDialog` was added in BUG-56 (before the #124 bottom-sheet redesign existed) as a quick `AlertDialog`-based fix, and was never migrated when #124 converted the other overflow actions to bottom sheets. `spec/VISUAL_SPEC.md` §Bottom sheets didn't list Change URL, and `spec/story-board/prototypes/subscriptions.jsx`'s per-feed overflow menu had no "Change URL" entry at all — BUG-56 was implemented straight in the Android app and never made it back into either spec artifact.
- **Fix:** Rebuilt `ChangeUrlDialog` on the `FeedBottomSheet` shell — same title/content/button-row chrome as every other sheet flow, with `testTagSuffix = "_change_url"` and an explicit `primaryTestTag = "change_url_save"` to preserve the pre-existing test tag. The inline error text keeps its `ToneErrFg`/12sp styling below the URL field. Updated both spec artifacts (`VISUAL_SPEC.md`, `subscriptions.jsx`) so the design reference and the implementation agree.
- **Validation:** `app/src/test/java/eu/monniot/feed/ui/subs/SubscriptionsScreenTest.kt` — extended the 3 existing BUG-56 Change-URL tests to assert `sheet_change_url`/`sheet_cancel_change_url` presence instead of AlertDialog text/dismiss semantics, and added `changeUrl_cancelDismissesSheetWithoutSaving`. `./gradlew :app:testDebugUnitTest` → 499 passed, 0 failed, 2 skipped (baseline 498/0/2). The story-board change has no automated test (manually-viewed design mockup, `spec/story-board/run.sh` serves it locally); verified it parses/transforms cleanly via `@babel/preset-react` and matches the `renameFeed` sheet's structure exactly.
