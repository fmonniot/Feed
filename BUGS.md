...
- **Status:** OPEN
- **Module:** `app/`
- **Files:** `app/src/main/java/eu/monniot/feed/ui/subs/FeedDialogs.kt` (`ChangeUrlDialog`, line ~40, built on Material3 `AlertDialog`; see the file's own header comment at line 25-28 flagging it as one of only two dialogs deliberately left outside the bottom-sheet shell)
...
- **Root cause:** `ChangeUrlDialog` was added in BUG-56 (before the #124 bottom-sheet redesign existed) as a quick `AlertDialog`-based fix, and was never migrated when #124 converted the other overflow actions to bottom sheets. `spec/VISUAL_SPEC.md` §Bottom sheets (line 523-525) enumerates the sheet-based flows and doesn't currently list Change URL, so the spec itself needs updating alongside the implementation.
- **Fix direction:** Rebuild `ChangeUrlDialog` on top of the shared `FeedBottomSheet` shell (same pattern as `RenameFeedSheet`/`FetchIntervalSheet` in the same package), preserving its existing validation/error-display behavior (`change_url_input`, `change_url_error`, `change_url_save` test tags). Update `spec/VISUAL_SPEC.md` §Bottom sheets to list "Change URL" alongside the other sheet-based flows. Also update the design reference itself: `spec/story-board/prototypes/subscriptions.jsx`'s per-feed overflow menu (`feedMenuEl`, line 768-777) has no "Change URL" entry at all — BUG-56 was implemented straight in the Android app and never made it back into the mockup. Add a `Change URL…` `mitem` (near `Rename…`, line 772) that opens a new `sheet.type === 'changeUrl'` case (alongside the `move`/`renameFeed`/`interval` cases at line 640-750) rendering an `SubMSheet` with a pre-filled URL `input`, matching the shape of the existing `renameFeed` sheet.
- **Validation:** Extend `app/src/test/java/eu/monniot/feed/ui/subs/SubscriptionsScreenTest.kt`'s existing Change-URL coverage (from BUG-56) to assert the sheet shell's tags (`sheet_primary`/`sheet_cancel` suffix, grab handle) are present instead of `AlertDialog` semantics. `./gradlew :app:testDebugUnitTest`, 0 failures. The story-board change has no automated test (it's a manually-viewed design mockup, `spec/story-board/run.sh` serves it locally) — verify visually that the new sheet opens and matches the other bottom sheets' shape.
