...
- **Fix direction:** Extend `FeedBottomSheet`'s background/border shape down to the bottom of the window (behind the nav bar), and apply `WindowInsets.navigationBars` (or `.navigationBarsPadding()`) only to the *content* padding, not the outer background bounds — mirroring how `ModalBottomSheet` normally handles this. Verify against both 3-button and gesture navigation modes.
- **Validation:** Manual verification (layout/inset behavior against the real system nav bar isn't practical to unit-test with Robolectric). Screenshot before/after on a device or emulator with gesture navigation enabled, confirming the sheet background is visible behind the (translucent) nav bar area while button tap targets stay in their current, correct position.
