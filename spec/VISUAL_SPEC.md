...
- **Feed rows** (10/32/40 padding) — full row, flex, 12px gap, 11/8 padding, 1px bottom `border` (none on last). A row mid-drag drops to `opacity: 0.4`. Left to right:
  - **Drag handle** — a 6-dot (2×3) grip in `ink3`, `cursor: grab`. The row is `draggable`; the handle drives drag-based organizing — dragging a row onto a rail category re-files the feed (SUBS-10), and it is also the **reordering** affordance. Drag is **web only**; Android has no drag (reorder is out of scope there — see [FEATURES.md](FEATURES.md) §Categories & feed management). **Story-board caveat:** the prototype draws the handle and wires only re-filing; it does **not** implement drop-to-reorder or persisted feed order (too much plumbing for a mock). Implement reorder from this contract, not from the story board — its absence there is a prototype shortcut, not a dropped feature.
...
- **Change URL** (BUG-56/BUG-60) pre-fills the sheet's text field with the feed's current source URL; a failed `PUT /v1/feeds/{id}` (invalid/unreachable URL) surfaces as an inline error line below the field and keeps the sheet open so the user can correct it.
