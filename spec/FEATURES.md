...
| Refresh interval (15m / 1h / 6h / manual) | ✓ | ✓ | 1h | **Client-side local polling only** — pulls an incremental sync delta (new articles, read-state changes, deletions) from our own server on this cadence and applies it to the local mirror; it does *not* re-download the full list. It does *not* control how often the server fetches feeds upstream — that is the server's **per-feed fetch interval**, now user-settable per feed on the Subscriptions screen (see §Categories & feed management and SUBS-11). `manual` disables the poll. The poll pauses while the client is backgrounded and resumes (with an immediate sync) on foreground; changing the interval takes effect live. |
...
## Categories & feed management

The **Subscriptions screen** (web sidebar link / Android **Feeds** tab) is the single place feeds and categories are managed. It is a **category manager**, not just a list, on **both platforms**.

- **Categories are first-class, user-managed objects.** They can be created, renamed, and deleted from the Subscriptions surface (the server already models categories — create / rename / delete / reorder, with `ON DELETE SET NULL` so deleting a category never deletes its feeds). **Ordering by drag is a web capability**: the web feed rows carry a **drag handle** for drag-based organizing (re-filing a feed onto a rail category, SUBS-10, and reordering). **Android has no drag, so reorder is out of scope there** — the Android list keeps a fixed order. *(The story board draws the handle but wires only re-filing — drop-to-reorder + persisted order is intentionally not prototyped; build reorder from this contract, not from the mock.)* On both platforms the rail/groups always show `All feeds` first (web), then the categories, with **Uncategorized last**.
- **"Uncategorized" is a permanent, locked category.** It cannot be renamed or deleted, always sorts last, and holds every feed whose category is unset (or whose folder matches no live category), so "no category" is always safe.
- **Deleting a category re-files its feeds, never unsubscribing them.** The user picks a target category for the orphaned feeds (an empty category deletes with no reassign step).
- **Every per-feed action lives in the row's `⋯` overflow**, on both platforms: **Refresh now**, **Move to category…**, **Rename…** (sets `custom_title`), **Fetch interval…**, **Pause / Resume updates**, and **Unsubscribe**. For a *failing* feed the row instead expands the diagnostic accordion (see §Feed errors), whose action set is a superset that adds `Retry once` / `Fix URL…` / `View raw ↗`.
- **Fetch interval is per feed** and controls the **server's upstream fetch cadence** for that feed (`fetch_interval_minutes`) — distinct from the global client **Refresh interval** poll in Settings (which only pulls a sync delta from our own server). The four exposed steps are **15m / 1h / 6h / Daily**.
- **Pause** stops the server fetching that feed upstream until the user resumes it; a paused feed keeps its cached articles, dims its avatar, and shows a "Paused" badge. This is user-initiated and distinct from the transient `paused` sync-footer state caused by rate-limiting (ERR-6).

### Layout & re-filing, per platform

- **Web** is a **two-pane manager** inside the Subscriptions route (to the right of the reading sidebar): a category **rail** (All feeds · each category · Uncategorized last · "+ New category") drives a **feed pane** for the selected category. A feed is re-filed either by **dragging its row onto a category in the rail** *or* via the row's `⋯` → **Move to category…**. Drag-and-drop is **web only**.
- **Android** keeps the **grouped-by-category list** (uppercase category headers, each non-locked header carrying a `⋯` for rename / delete). The app-bar carries **search / add-feed / overflow (+ New category)**. There is **no drag-and-drop**; re-filing and all edits go through the `⋯` menus and **bottom sheets**. Adding a feed lands it in **Uncategorized** (the user moves it afterward).

Category and feed edits are **immediately reflected in the reading sidebar / Feeds tab** — they share one category + feed model. Visual detail is in [VISUAL_SPEC.md](VISUAL_SPEC.md) §Web · Subscriptions and §Mobile (Android) · Feeds.
...
| FEED-2 | web | Populated server with at least one feed | Click a feed (subscription) in the sidebar folder list | List filters to that subscription's items — a **local filter against the mirror**, no per-feed server fetch (Article sync contract). Web URL = `/feed/:subscriptionId` (the server's feed id). **Android: not supported yet** — the redesigned Feeds tab is a category manager, not a browse surface, and there is no agreed design for reaching a single feed's article list on mobile; the Android nav layer stays Unread / All / Feeds / Settings until one exists. |
...
| FEED-6 | android | Article list with content | Pull down on any article list (Unread / All — Android has no per-feed list yet, FEED-2) | Triggers a refresh; spinner shown; list refreshes; error path lands in ERR-1. |
...
| FEED-13 | web | Feed (subscription) view with ≥ 1 unread article | Click the `✓ Mark all read` action in the article-list header | Every unread article in the current list is marked read (`PUT /v1/articles/{id}/read` per article), their unread dots + per-row `✓` buttons disappear, and the Unread badge decrements by that count. The header action is present **only when the current view has ≥ 1 unread item** — the same visibility rule as the per-row `✓` (FEED-8). It is view-generic: the same action clears whichever list is open (per-feed, **Unread**, or **All**), scoped to the articles currently listed. In the Unread view the rows drop out on the spot (FEED-8 TODO-list behaviour). No confirmation dialog — the action is non-destructive and reversible via FEED-14. Android is out of scope for this action for now; this is web-only. |
| FEED-14 | web | Immediately after FEED-13, before navigating away | Click the `↩ Undo` action that has replaced `Mark all read` in the same header slot | The just-cleared articles return to unread — dots, per-row `✓` buttons, and every unread count restored (`PUT /v1/articles/{id}/read` with `is_read=false` for each). The Undo occupies the **same header slot** as the action it reverses for ~6s, then the slot reverts to `Mark all read` (or to nothing, if the view now has no unread); navigating to another view also dismisses it. The button flip is the only confirmation — there is no status line or toast (web has no snackbar). |
...
| ID | Platforms | Setup | Steps | Expected |
|---|---|---|---|---|
| SUBS-1 | both | Server has feeds across multiple categories, including at least one empty category | Open Subscriptions (web sidebar link / Android `Feeds` tab) | Every feed is shown under its category. **Web:** a category rail (All feeds · each category · Uncategorized last) drives a feed pane listing the selected category's feeds. **Android:** feeds are grouped under uppercase category headers. Empty categories still appear (so they can be renamed / deleted) — a zero-count rail row on web, an empty group on Android. |
| SUBS-2 | both | Valid RSS URL on hand | Click/tap "+ Add feed", paste URL, submit | New feed row appears; `POST /v1/feeds` returns 201; nav count increments. **Web:** the feed is created in the currently-selected category (Uncategorized when "All feeds" or a locked category is active). **Android:** the feed lands in Uncategorized, with a sheet note to move it afterward. |
| SUBS-3 | both | Multiple feeds, at least one whose name contains the substring being typed | Type that substring in the search box | Only matching feed rows visible; filtering is client-side, no server call. On web the pane search scopes to the selected category; the rail has its own separate "Filter categories…" box. |
| SUBS-4 | both | Existing feed | Open the feed's `⋯` overflow → Rename… → enter new name → save | Row updates inline; `PUT /v1/feeds/{id}` with `custom_title = "<new>"`. The `⋯` menu also carries Refresh now / Move to category… / Fetch interval… / Pause-Resume / Unsubscribe (SUBS-10–SUBS-12, SUBS-16, SUBS-5). **Web requirements:** the overflow dropdown must render above adjacent rows (not be clipped by the list container), and the rename input must be prepopulated with the current name and pre-selected. |
| SUBS-5 | both | Existing feed with at least one article | Feed `⋯` → Unsubscribe → confirm | Confirms (`Unsubscribe from "{name}"? Its articles will be removed.`), then `DELETE /v1/feeds/{id}`; the feed and its articles vanish from the article list and the subscriptions surface. |
...
| SUBS-10 | both | Feed filed under category A; at least one other category B exists | Move the feed to B — **web:** drag its row onto B in the rail, *or* `⋯` → Move to category… → B; **android:** `⋯` → Move to category… (bottom sheet) → B → Move | The feed leaves A and appears under B on both the Subscriptions surface and the reading sidebar / Feeds tab; `PUT /v1/feeds/{id}` sets the new category. The Move submenu/sheet marks the feed's current category and offers "+ New category…" to create-and-move in one step. **Drag-and-drop is web only** — Android has no drag affordance and re-files via the menu. |
| SUBS-11 | both | Existing feed | `⋯` → Fetch interval… → pick a value (15m / 1h / 6h / Daily) | The feed's upstream fetch cadence is persisted server-side (`PUT /v1/feeds/{id}` with `fetch_interval_minutes`); the chosen value shows as the trailing hint on the menu item. This is the **server's per-feed upstream cadence**, independent of the global client Refresh-interval poll (SET-9). |
| SUBS-12 | both | Active (non-paused) feed | `⋯` → Pause updates; later `⋯` → Resume updates | Pause: the server stops fetching the feed upstream (`PUT /v1/feeds/{id}` with paused=true); the row dims its avatar and shows a "Paused" badge, and the menu item flips to "Resume updates". Cached articles remain readable. Resume clears the paused state and the badge. Distinct from the rate-limit `paused` footer state (ERR-6). |
| SUBS-13 | both | Subscriptions open | Create a category — **web:** rail footer "+ New category" → type name → Enter; **android:** app-bar `⋯` → "+ New category…" → type → Create | A new empty category appears in the rail (web) / as a group header (android) and becomes a Move-to target; the server persists it. Creating a category whose name duplicates an existing one is a no-op. |
| SUBS-14 | both | Existing non-locked category | Category `⋯` → Rename… → new name → save | The category and every feed filed under it update to the new name on the Subscriptions surface and the reading sidebar / Feeds tab; the server persists the rename. "Uncategorized" has no `⋯` and cannot be renamed. |
| SUBS-15 | both | Non-locked category with ≥1 feed, plus at least one other category | Category `⋯` → Delete category… → pick a target category → confirm | A confirm surface (web modal / android sheet) states the feeds are kept and asks where they go. On confirm the category is deleted and its feeds are re-filed to the chosen target (server `ON DELETE SET NULL` + move); **no feed is unsubscribed**. An empty category deletes without a reassign step. "Uncategorized" cannot be deleted. |
| SUBS-16 | both | Existing feed | `⋯` → Refresh now | Fires an upstream fetch for that one feed (`POST /v1/feeds/{id}/refresh`); the row shows an in-progress spinner, then any newly-fetched articles arrive via the next sync delta. This is the per-feed analog of the global refresh gesture (SET-10). |
