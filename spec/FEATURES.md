...
- **Categories are first-class, user-managed objects.** They can be created, renamed, and deleted from the Subscriptions surface. The server already models categories (create / rename / delete / reorder, with `ON DELETE SET NULL` so deleting a category never deletes its feeds); the client exposes create / rename / delete but **not reorder** — category order is fixed (`All feeds` first on web, then the categories, with **Uncategorized last**). Reorder is out of scope for now.
