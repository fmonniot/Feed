...
Backlog of tickets. Reference tickets by their numeric ID (e.g. "work on #3").
Numeric IDs are stable; gaps from closed/superseded tickets are intentional.

Status legend: `[ ]` open · `[~]` in progress · `[x]` done · `[-]` closed without
action · `[?]` needs verification

Tickets are listed **in ID order** below (oldest filed first, newest last). This file
...
### #123 — Subscriptions redesign: web two-pane category manager `[x]`
...
**Resolution:** Rebuilt [SubscriptionsScreen.kt](web/src/jsMain/kotlin/eu/monniot/feed/web/ui/subs/SubscriptionsScreen.kt) as the three-column layout (reading sidebar + 248px category rail + feed pane), consuming #122's shared `FeedViewModel` category/per-feed actions directly — no new shared logic. The rail (`renderRail`/`renderRailList`) shows All feeds → categories by position → locked Uncategorized last, each with a live feed count, a "Filter categories…" box, and a footer "+ New category" input; non-locked rows carry a `⋯` → Rename…/Delete category… menu, with rename swapping to an inline input. The feed pane (`renderPane`/`renderPaneFeedList`) carries the category-name H1 + count (`paneCountLabel`: "{N} feeds" / "showing {X} of {Y}" while searching), a "+ Add feed" form that files new feeds into the selected category, and pane search. Feed rows (`feedRow`) gained a drag handle, a paused badge, and the `{N} new` / spinner swap during a per-feed refresh; the `⋯` overflow now opens a **Move to category…** submenu in place (radio rows + "+ New category…") instead of the old `window.prompt`-based "Set folder…". The #79-cluster feed-error surface (`renderErrorBanner`/`feedErrorAccordion`/broken-row badge) is unchanged and now lives inside the pane; a new dismissible banner also surfaces the shared `feedsError` flow (wired to the web client for the first time — previously only Android read it, per commit b3265f7).

**Re-filing + reorder (SUBS-10):** both drag-a-row-onto-a-rail-category and `⋯` → Move to category… call `setFeedCategory`. **Drag-to-reorder was the one piece not in the story board** (`subscriptions.jsx`'s `SubHandle` comment explicitly says so) — implemented by making non-locked rail rows themselves `draggable`; dropping one onto another computes the new order (`reorderedCategoryIds`, pure/unit-tested) and persists it via `reorderCategories(orderedCategoryIds)`. Delete-category (rail ⋯ → Delete category…) opens an in-pane reassign modal (`showDeleteCategoryModal`) mirroring the VISUAL_SPEC shell; confirming calls `deleteCategory(id, reassignTo)` — no feed is ever unsubscribed (verified by a test asserting zero `deleteFeed` calls after a delete-with-reassign).

**Notable bug caught by the new tests:** committing the inline "+ New category" / category-rename inputs on Enter fired `createCategory`/`renameCategory` **twice** — the commit handler's own re-render detaches the still-focused input, and browsers synchronously fire a native `blur` on a focused element removed from the DOM, re-entering the same `blur`-triggered commit path. Fixed with a per-commit one-shot guard flag in both `wireRailChrome` and the rail rename wiring.

**Test plan:** `./gradlew :web:jsTest -PskipServerBuild` → 581 passed / 0 failed / 0 skipped (baseline 551). Added `SubsRailLogicTest` (18 pure-function tests: `feedsForSelection`, `railSelectionTitle`, `paneCountLabel`, `filterCategories`, `reorderedCategoryIds`, `initialRailSelection`) and `SubsScreenIntegrationTest` (11 DOM tests mounting the real screen against a call-recording `SubsFakeFeedRepository`, driving real clicks plus synthetic `dragstart`/`dragover`/`drop` events — rail rendering/ordering/counts, category create/rename, move via menu and via drag, reorder-via-drag + persistence, delete-with-reassign never unsubscribing, and a per-feed pause action). `SubsCopyAlignmentTest` was rewritten for the new pane count copy ("{N} feeds" / "showing {X} of {Y}") replacing the retired flat-list "{N} of {M}" format; `SubsFeedErrorTest`'s two overflow-menu action-set assertions were updated from `set-folder` to `move-to-category`. All other pre-existing subs test files (`SubsScreenRenderTest`, `SubsOverflowMenuTest`, `SubsFetchIntervalTest`, `SubsFixUrlDialogTest`, `SubsChangeUrlActionTest`, `SubsAddFeedErrorTest`, `SubsSearchFilterTest`) needed no changes — their target functions/DOM ids were kept stable through the rebuild.

---

### #124 — Subscriptions redesign: Android Feeds-tab category manager `[x]`
...
**Resolution:** `SubscriptionsScreen`/`SubscriptionsScreenContent` (`app/src/main/java/eu/monniot/feed/ui/subs/SubscriptionsScreen.kt`) now group feeds into a `CategoryGroup` list built from the shared `categories` StateFlow: every real category renders (even empty, with a "Nothing here yet." row) and carries a header `⋯` (Rename…/Delete category…); the locked "Uncategorized" bucket (feeds with no live `category_id`) always sorts last with no `⋯`. A brand-new account (zero categories, zero feeds) still shows the plain empty-state CTA instead of a bare header. Six flows moved from `AlertDialog`s to a shared `FeedBottomSheet` shell (scrim + grab handle + title + body + Cancel/primary button row, built as a custom full-screen `Dialog` like the pre-existing `AddFeedDialog` rather than Material3's `ModalBottomSheet`, so it stays inside the normal semantics tree for Robolectric and keeps the exact VISUAL_SPEC chrome): **Move to category…** (radio rows incl. "current"/"default" notes + a "+ New category…" link), **Rename feed**, **Fetch interval** (immediate-select radio rows, no separate confirm — preserves the prior UX), **New category**, **Rename category** (prefilled), and **Delete category** (reassign radio list defaulting to Uncategorized for non-empty categories; empty categories delete directly with no reassign step). Refresh / Pause-Resume / Unsubscribe remain inline menu actions; Change URL (BUG-56) and the Unsubscribe confirm stay plain `AlertDialog`s (outside the #124 bottom-sheet list). Add Feed is now a bottom sheet with a static "Added to 'Uncategorized' — move it to another category afterward from the feed's ⋯ menu" note (SUBS-2); the sheet's button-row min-height still matches `ButtonSize.Medium` app-wide. `MainTabShell.kt`'s Feeds-tab header gained a second app-bar icon (`feeds_overflow_action`, 32dp, matching the existing Add button) opening a one-item menu ("+ New category…") that hoists a `showNewCategorySheet` boolean into `SubscriptionsScreen`, mirroring the existing `showAddFeedDialog` reset-on-consume pattern; the header subtitle is now `feedsTabSubtitle(feedCount, categoryCount)` ("{N} subscriptions · {M} categories"). The dedicated search-icon toggle from #116/#117 was kept in place inside the content (not relocated into the title row) to avoid re-forking its already-tested affordance — functionally equivalent, one tap from the Feeds screen. `app/src/main/java/eu/monniot/feed/FeedViewModel.kt` gained thin `createCategory`/`renameCategory`/`deleteCategory` delegations to the #122 shared `FeedViewModel` (reorder was intentionally left off — Android has no drag, per #122/#124 scope). Tests: 17 new cases in `SubscriptionsScreenTest.kt` (empty-category headers, locked Uncategorized, category CRUD sheets incl. both reassign paths and the empty-category direct-delete path, Move-sheet current/default notes and its New-category link, Add-feed sheet note, plus four PR-review follow-ups: RenameFeedSheet's clear-title regression, MoveToCategorySheet's dangling-categoryId normalization, a many-categories scroll check for `FeedBottomSheet`'s height cap, and the category overflow button's content description) plus 3 new `feedsTabSubtitle` cases in `MarkAllReadTest.kt`; several existing tests were updated in place for the AlertDialog→bottom-sheet flows they exercise (Move/Rename-feed/Fetch-interval) rather than forked. `./gradlew :app:testDebugUnitTest -PskipServerBuild` — 496 tests completed, 0 failed, 2 skipped (the `@Ignore`'d PullToRefresh gesture tests).
...
### #132 — Add partial index on `articles.link_status` for the probe-job queue scan `[ ]`
...
**Note (2026-07-10):** renumbered from the original `#115` — that ID collided with the unrelated, already-shipped "Web: unread badge on sidebar source items" ticket. Same collision class as the one #130 fixed for `#122`; see this repo's BUGS.md/TICKETS.md reorg session notes.
...
### #133 — Web: drag-to-reorder feeds within a category (persisted order) `[x]`
...
**Resolution:** **Server:** added migration v21 in `server/src/db.rs` — `feeds.position INTEGER NOT NULL DEFAULT 0` + `idx_feeds_position`, backfilled per-category (including the uncategorized group) by id order via a correlated-subquery `UPDATE`. `get_all_feeds`, `get_feeds_with_unread`, `get_feeds_by_category`, and `get_feeds_by_category_with_unread` all gained `ORDER BY position`; `add_feed` now appends new (uncategorized) feeds after the current max uncategorized position, mirroring `create_category`. Added `Database::update_feed_positions` (mirrors `update_category_positions`) and a `POST /feeds/reorder` route/handler/DTOs (`ReorderFeedsRequest`/`FeedPosition`, mirroring the category reorder shape), documented in `spec/API_DOCUMENTATION.md`. Three existing rollback-based migration tests (`test_migration_11_drops_refresh_tokens_table`, `test_migration_19_backfills_pre_existing_errors`, `test_sync_migration_backfill_sets_seq_equals_id`) needed their rollback blocks extended to also drop the new `position` column/index, since they reopen the DB at an older schema version and would otherwise hit "duplicate column name" once migration v21 re-runs. Added `test_update_feed_positions` and two `test_migration_21_*` tests (existence + backfill round-trip) to `db_tests.rs`. **Shared:** added `FeedApi.reorderFeeds` + `FeedPosition`/`ReorderFeedsRequest` models (snake_case, mirroring the server's serde types), `FeedRepository.reorderFeeds`/`SharedFeedRepository` impl, and `FeedViewModel.reorderFeeds(orderedFeedIds)` (mutate then `loadFeeds()`, same idiom as `reorderCategories`). Added a `position: Int = 0` field (defaulted, so the ~15 existing `Feed(...)`/`FeedUiItem(...)` test fixtures across the codebase didn't need updating) to both the shared `Feed` API model and `FeedUiItem`, threaded through `loadFeeds()`. Covered by `SharedFeedRepositoryTest.reorderFeeds_sendsPositionsMatchingListOrder`, `FeedViewModelCategoryManagementTest.reorderFeeds_delegatesWithOrderedIdsAndRefreshesFeeds`, and `FeedViewModelErrorLoggingTest.reorderFeedsLogsExceptionBeforeMapping`; the 11 other `FeedRepository` fakes across `shared`/`web` tests got a no-op or (for `SubsFakeFeedRepository`) a real in-memory `reorderFeeds` override so they kept compiling. **Web:** added a pure `reorderedFeedIds` helper in `SubsPane.kt` (mirrors `reorderedCategoryIds`'s direction-aware insert, computed from the *unfiltered* per-selection feed list so a pane search filter doesn't corrupt the order), and made each pane feed row (`[data-feed-row]`) a `dragover`/`dragleave`/`drop` target reading `state.dragFeedId` — the same in-memory drag payload the rail already uses, so re-filing a feed onto a rail category (SUBS-10) is unaffected (disjoint DOM elements, verified by a regression test). The drag still only *starts* from the grip handle, not the row, preserving text selection. Tests: 5 pure-logic cases in `SubsRailLogicTest.kt` (mirroring the 5 `reorderedCategoryIds` cases) plus 3 DOM integration tests in `SubsScreenIntegrationTest.kt` (sibling-row reorder + persistence, self-drop no-op, rail re-file regression guard). Final counts: server 306 passed (baseline 303, +3), shared-js 425 passed (baseline 422, +3), web 600 passed (baseline 592, +8), all 0 failed.

---

### #134 — Android: Feeds search field takes up fixed real estate and never collapses on scroll `[x]`

Once the Search icon is toggled on (see BUG-61 for the icon's own misplacement — the two should probably be picked up together), the inline filter field (`SubscriptionsScreen.kt`, `search_field` row, ~line 333-365) sits as a permanently-visible fixed-height row above the category list. On a screen with many categories/feeds it eats into the visible list area for the whole session and never scrolls away or collapses, unlike e.g. the error summary banner which is contextual.

**Design decision (resolved):** rather than making the inline field collapse on scroll, the search field is promoted to a full-screen **search mode**. Tapping the app-bar `⌕` swaps the entire "Feeds" title header for a search-bar row (back chevron `‹` + the canonical filter box), filters the grouped list live, and the back chevron clears the query and returns to browse. This reclaims all the fixed real estate the old inline row consumed (the header height is reused, nothing is added below it) and sidesteps the collapsing-toolbar complexity entirely. The behavior is prescribed in `spec/VISUAL_SPEC.md` §Mobile (Android) · Feeds ("Search mode") and the app-bar `⌕` action, and prototyped in `spec/story-board/prototypes/subscriptions.jsx` (`SubsMobile`, `searchMode`). See commit `a6e30540`.

**Acceptance criteria**
- **Design & spec:** ✅ chosen behavior (full-screen search mode) recorded in `spec/VISUAL_SPEC.md` §Mobile (Android) · Feeds and reflected in the storyboard prototype.
- **Implementation:** replace the inline `search_field` row in `SubscriptionsScreen.kt` with the search-mode top bar: tapping `⌕` enters a mode that swaps the title header (`SubMHeader` equivalent) for the search-bar row (back `‹` in `accent` + filter box with `⌕` glyph, input, and a trailing clear `✕` that shows once the query is non-empty), auto-focusing the input on entry (preserve the one-tap-keyboard behavior from #116/#117) and hiding the other app-bar actions (`+`, `⋯`) while active. The back chevron and an empty→exit both clear the query. Keep the app-bar `⌕` a plain action (no toggled-open styling).
- **Tests:** `app/src/test/java/eu/monniot/feed/ui/subs/SubscriptionsScreenTest.kt` covers entering search mode from `⌕`, that the header title is replaced by the search bar, live filtering of the list, the clear `✕` visibility/behavior, and that back exits and clears the query. `./gradlew :app:testDebugUnitTest`, 0 failures.

**Resolution:** Promoted search to a full-screen mode owned by [MainTabShell.kt](app/src/main/java/eu/monniot/feed/ui/shell/MainTabShell.kt). Removed BUG-61's inline `search_field` row (and its focus wiring) from [SubscriptionsScreen.kt](app/src/main/java/eu/monniot/feed/ui/subs/SubscriptionsScreen.kt), which now only *consumes* the hoisted `searchQuery` to filter the grouped list (dropped the `searchExpanded`/`onSearchQueryChange` params — leaving search mode clears the query, so blank ⇒ not searching). `FeedsSearchToggleAction` is now a plain action (`onEnter`, no `expanded`/toggled-open `accentSoft` styling); tapping it flips `feedsSearchActive`, and the Feeds `topBar` then renders the new `FeedsSearchTopBar` (back `‹` accent glyph + 40dp tap target, matching RawResponseInspectorScreen's `‹`; canonical filter box — `panel`/`borderStrong`, `⌕`, input placeholder "Search feeds…", trailing clear `✕` shown only when non-empty) in place of the "Feeds" `TabScreenHeader`, so the swapped header reuses the same real estate and the other app-bar actions (`+`, `⋯`) vanish while active. The input auto-focuses on entry (`FocusRequester` + `LaunchedEffect(Unit)`), preserving the one-tap-keyboard behavior from #116/#117; the back chevron and the system/predictive back button (`BackHandler`) both exit and clear the query. Deleted the now-obsolete `toggleFeedsSearch` pure function and `FeedsSearchToggleTest.kt` (the ⌕ is no longer a toggle); updated `TabScreenHeaderTest.kt` to the new `onEnter` signature. Tests: 4 new tests in `SubscriptionsScreenTest.kt` (enter-mode header swap + hidden `+`, auto-focus, clear-`✕` visibility/in-place empty, back exits+clears+re-enters empty), rewrote the old toggle-collapse test to exit via `‹`, and repointed the existing filter tests through the new mode. `./gradlew :app:testDebugUnitTest -PskipServerBuild`: 514 passed, 0 failed, 2 skipped. Note: the search-bar row's exact pixel metrics (min-height 54, insets) are manual-visual per spec — no live device/AVD was available in this session, so styling was matched to `spec/story-board/prototypes/subscriptions.jsx`'s `searchMode` bar by construction rather than screenshot-compared.

---

### #135 — Android: replace all AlertDialog with FeedBottomSheet `[x]`
...
**Resolution:** Audit (`grep -rn AlertDialog app/src/main/`) found three remaining Material3 `AlertDialog`s after BUG-60 already converted Change URL: `DeleteConfirmDialog` in [FeedDialogs.kt](app/src/main/java/eu/monniot/feed/ui/subs/FeedDialogs.kt) (feed Delete/Unsubscribe confirm — one composable serves both the broken-row Delete action and the healthy-row overflow menu's Unsubscribe item, which previously fired inline via `AlertDialog` directly), `MarkAllReadConfirmDialog` in [MainTabShell.kt](app/src/main/java/eu/monniot/feed/ui/shell/MainTabShell.kt) (ticket #9's threshold-guarded confirm), and the OPML-import-result dialog in [SettingsScreen.kt](app/src/main/java/eu/monniot/feed/ui/settings/SettingsScreen.kt). Rebuilt all three on `FeedBottomSheet` (internal, cross-package accessible within `app`), following BUG-60's idiom exactly: kept each dialog's original function name/signature and existing confirm/cancel test tags where a prior test already asserted them (`mark_all_read_confirm`/`_cancel`), and added new `sheet_<suffix>` shell tags for the two that didn't have dedicated bottom-sheet coverage yet (`sheet_delete_feed`/`delete_feed_confirm`/`delete_feed_cancel`, `sheet_opml_result`/`opml_result_ok`). Delete/Unsubscribe uses the shell's `primaryDanger` styling (same as Delete category); Mark-all-read uses a non-danger primary (bulk but not destructive in the same sense); the OPML result sheet is acknowledgement-only, so it omits `primaryLabel` entirely and repurposes the shell's secondary-button slot with `secondaryLabel = "OK"` instead of the usual Cancel — no `FeedBottomSheet` API changes were needed since that flexibility already existed (nullable `primaryLabel`, customizable `secondaryLabel`). Updated [spec/story-board/prototypes/subscriptions.jsx](spec/story-board/prototypes/subscriptions.jsx) so the mobile Unsubscribe action opens a `deleteFeed` mock sheet (matching `SubMSheet`'s existing danger-confirm pattern used by Delete category) instead of firing a native `confirm()`. Added a new `spec/VISUAL_SPEC.md` §Mobile (Android) · Dialogs section documenting the rationale (one interaction model, one place to define scrim/corner/danger styling, danger legibility) and enumerating all three migrated flows. Tests: 3 new tests in `SubscriptionsScreenTest.kt` (open/confirm/cancel for the Delete/Unsubscribe sheet), 2 new tests in `SettingsScreenTest.kt` (OPML sheet title/OK button, OK-dismiss wiring), 1 new test in `MarkAllReadTest.kt` (shared-shell tag pin) — the existing `dialog_confirmInvokesCallback`/`dialog_cancelInvokesDismissWithoutConfirming` and `accordion_unsubscribeAction_deletesViaConfirmDialog`/`brokenFeedRow_overflowMenu_deleteInvokesCallback` tests already exercised the converted composables' new implementation unchanged (same test tags/wording). `./gradlew :app:testDebugUnitTest -PskipServerBuild`: 507 passed, 0 failed, 2 skipped (baseline 501, +6 new tests).
